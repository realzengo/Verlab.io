...
export function createMcpServer(userId: string): McpServer {
  const server = new McpServer(
    ...
    {
      instructions:
        "Verlab tools for short-form video creators: trending niche research, single-creator content-strategy analysis (real transcripts, not just metadata), script generation, AI voiceover generation (text-to-speech, 30 catalog voices), image generation, transcript extraction, and video downloads. Script/voiceover/image/transcript/download/creator-analysis tools charge credits. Do NOT call get_credit_balance proactively before running these. Only call it when the user explicitly asks about their credits/balance, or when a charging tool call fails due to insufficient credits.",
    }
  );
