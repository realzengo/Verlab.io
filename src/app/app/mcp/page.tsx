    ...
    <div className="mx-auto mt-10 max-w-2xl text-left">
      ...
      <div className="mt-3 flex flex-col gap-2">
        {connections.map((connection) => (
          ...
          >
            <span className="text-sm font-medium text-heading">{connection.clientName || "Unnamed app"}</span>
            <Button
              variant="secondary"
              size="sm"
              onClick={() => handleDisconnect(connection.clientId)}
              disabled={revokingId === connection.clientId}
            >
              {revokingId === connection.clientId ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : "Disconnect"}
            </Button>
          </div>
        ))}
      </div>
