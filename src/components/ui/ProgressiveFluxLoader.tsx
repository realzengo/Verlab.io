...
export interface ProgressiveFluxLoaderProps {
   ...
   * `var(--color-primary)` to follow the theme).
   */
  gradient?: string;
...
// `--flux-to`, or override the whole fill with the `gradient` prop.
...
function FluxLabel({ label, reduced, className }: FluxLabelProps) {
  const base = cn(
    "absolute inset-0 flex items-center justify-center text-center text-2xl font-bold tracking-tight text-heading sm:text-3xl",
    className,
  );
...
export function ProgressiveFluxLoader({
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          "relative h-5 w-full overflow-hidden rounded-full bg-app shadow-[inset_0_2px_3px_rgba(0,0,0,0.09),inset_0_-1px_2px_rgba(255,255,255,0.7)] dark:shadow-[inset_0_2px_3px_rgba(0,0,0,0.45),inset_0_-1px_2px_rgba(255,255,255,0.05)]",
          barClassName,
        )}
        role="progressbar"
        aria-valuemin={0}
